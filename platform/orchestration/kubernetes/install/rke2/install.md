...
# Проверить статус сервиса, чтобы убедиться что установка прошла успешно
systemctl status rke2-server

# ВАЖНО: НЕ запускаем сейчас! Сначала нужно создать конфиг (если не первая нода)!
# Запуск и enable будет после создания config.yaml (см. разделы ниже)
```
...
# ВАЖНО: НЕ запускаем сейчас! Сначала нужно создать конфиг!
# Запуск и enable будет после создания config.yaml (см. раздел "Расширение кластера")
```
...
# ВАЖНО: Убедиться что rke2-server отключен (не должен запускаться!)
systemctl is-enabled rke2-server 2>/dev/null && systemctl disable rke2-server || echo "✅ rke2-server уже отключен"
