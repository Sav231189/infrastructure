...
- **[Kubernetes / Helm](install/helm-install.md)** - установка в кластер K8s (рекомендуется)
- **[Docker Compose](install.md)** - установка на standalone сервере
...
1. NGINX с SSL/TLS проксирует запросы с `https://woodpecker.yourdomain.com` на внутренний сервис
2. Woodpecker API и Web UI работают на одном порту
