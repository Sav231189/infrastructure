# Woodpecker CI (Kubernetes / Helm)

## Что это?

Бесплатный CI/CD (форк Drone). Автоматически собирает Docker образы при пуше в GitHub и публикует в Harbor.

**Компоненты:**

- **Server** - веб-интерфейс и API
- **Agent** - выполняет сборки (масштабируется)
- **SQLite** - встроенная база данных

---

## Шаг 4: Настройка домена в NPM

Настройте прокси в Nginx Proxy Manager:

1. **Proxy Hosts** → **Add Proxy Host**
2. Заполните:
   - **Domain Names:** `woodpecker.stroy-track.ru`
   - **Scheme:** `http` (не https!)
   - **Forward Hostname/IP:** IP вашего Ingress (узнать в Lens: **Network** → **Services** → `ingress-nginx-controller`)
   - **Forward Port:** `80`
   - **Websockets Support:** ✅ (включить)
3. Вкладка **SSL:**
   - **SSL Certificate:** Request a new Let's Encrypt Certificate
   - **Force SSL:** ✅
   - **Email:** ваш email
4. **Save**

---

## Шаг 2: Установка через Lens

1. **Lens** → ваш кластер
2. **Helm** (левое меню) → **Releases**
3. **"+"** (Install Chart)
4. **Chart:** `oci://ghcr.io/woodpecker-ci/helm/woodpecker`
5. **Namespace:** создайте `woodpecker`
6. **Release name:** `woodpecker`
7. Вставьте **Values.yaml** ниже
8. **Измените** только URL (домен)
9. **Install**

### Values.yaml

⚠️ **Измените только URL на ваш домен:**

```yaml
server:
  host: "https://woodpecker.stroy-track.ru" # ← ЗАМЕНИТЬ: ваш домен
  logLevel: "info"
  replicas: 1
  resources:
    requests:
      cpu: 100m
      memory: 128Mi
    limits:
      cpu: 500m
      memory: 512Mi
  persistence:
    enabled: true
    storageClass: "ceph-rbd"
    size: 5Gi

agent:
  replicas: 2
  resources:
    requests:
      cpu: 200m
      memory: 256Mi
    limits:
      cpu: 2000m
      memory: 2Gi

ingress:
  enabled: true
  className: "nginx"
  hosts:
    - host: woodpecker.stroy-track.ru # ← (тот же домен)
      paths:
        - path: /
          pathType: Prefix
  tls: []
  annotations:
    nginx.ingress.kubernetes.io/ssl-redirect: "false"
    nginx.ingress.kubernetes.io/proxy-read-timeout: "3600"
    nginx.ingress.kubernetes.io/proxy-send-timeout: "3600"
    nginx.ingress.kubernetes.io/proxy-body-size: "100m"

database:
  type: sqlite
```

---

## Шаг 3: Проверка через Lens

1. **Lens** → **Workloads** → **Pods**
2. Namespace: `woodpecker`
3. Должны быть **Running**:
   - `woodpecker-server-xxxxx` (1 шт)
   - `woodpecker-agent-xxxxx` (2 шт)

**Если поды не запускаются:**

- Кликните на под → **Logs** → смотрите ошибку

---

## Шаг 1: Создать GitHub OAuth App

1. Откройте https://github.com/settings/developers
2. **New OAuth App**
3. Заполните:
   - **Application name:** `Woodpecker CI`
   - **Homepage URL:** `https://woodpecker.stroy-track.ru`
   - **Authorization callback URL:** `https://woodpecker.stroy-track.ru/authorize`
4. **Register application**
5. Скопируйте **Client ID** и **Client Secret** (понадобятся в UI Woodpecker CI позже)

---

## Шаг 5: Первый вход и настройка GitHub

### 1. Откройте Woodpecker UI

1. Откройте `https://woodpecker.stroy-track.ru`
2. Увидите страницу настройки

### 2. Настройте GitHub OAuth в UI

1. На странице входа выберите **GitHub**
2. Заполните форму:
   - **Client ID:** вставьте Client ID из шага 1
   - **Client Secret:** вставьте Client Secret из шага 1
3. Нажмите **Continue** или **Save**
4. Вас перенаправит на GitHub для авторизации
5. Нажмите **Authorize**
6. Вы в Woodpecker! 🎉

> **Важно:** Первый вошедший пользователь автоматически получит права администратора.

> **Альтернатива:** Если кнопка GitHub не появляется, перейдите в **Admin Settings** (иконка шестеренки) → **Configure** → добавьте GitHub настройки там.

---

## Работа в UI: Подключение репозитория

### 1. Синхронизировать репозитории из GitHub

1. **Repositories** (левое меню)
2. Кнопка **Reload repositories** (справа сверху, иконка обновления)
3. Подождите 3-5 секунд (идет синхронизация)

### 2. Включить CI для репозитория

1. Найдите нужный репозиторий в списке
2. Нажмите **Enable**
3. Репозиторий активирован ✅

### 3. Настройки репозитория

Кликните на репозиторий → **Settings** (вкладка сверху):

**General:**

- **Trusted:** ✅ (разрешить Docker и монтирование volumes)
- **Protected:** ✅ (только admin может менять настройки)
- **Timeout:** `3600` (1 час макс. на сборку)

**Сохраните изменения**

---

## Работа в UI: Добавление секретов

Секреты для Harbor/Docker Registry:

### 1. Откройте настройки репозитория

1. **Repositories** → выберите репозиторий
2. **Settings** → **Secrets** (вкладка)

### 2. Добавьте секреты

Нажимайте **New secret** для каждого:

**Секрет 1: DOCKER_REGISTRY**

- **Name:** `DOCKER_REGISTRY`
- **Value:** `harbor.stroy-track.ru`
- **Events:** ✅ все галочки (push, tag, pull_request, deployment, cron, manual)
- **Save**

**Секрет 2: DOCKER_USERNAME**

- **Name:** `DOCKER_USERNAME`
- **Value:** `admin` (или ваш логин от Harbor)
- **Events:** ✅ все галочки
- **Save**

**Секрет 3: DOCKER_PASSWORD**

- **Name:** `DOCKER_PASSWORD`
- **Value:** ваш пароль от Harbor
- **Events:** ✅ все галочки
- **Save**

### 3. Проверка

Секреты отображаются в списке (значения скрыты `****`).

---

## Работа в UI: Создание пайплайна

### 1. Создайте файл `.woodpecker.yml`

В корне вашего GitHub репозитория создайте файл `.woodpecker.yml`:

```yaml
when:
  branch:
    - stage
    - prod

steps:
  build-docker:
    image: plugins/docker
    settings:
      registry:
        from_secret: DOCKER_REGISTRY
      username:
        from_secret: DOCKER_USERNAME
      password:
        from_secret: DOCKER_PASSWORD
      repo: harbor.stroy-track.ru/stroytrack/${CI_REPO_NAME}
      tags:
        - ${CI_COMMIT_BRANCH}-${CI_COMMIT_SHA:0:8}
        - ${CI_COMMIT_BRANCH}-latest
      dockerfile: Dockerfile
```

### 2. Запушьте в GitHub

```bash
git add .woodpecker.yml
git commit -m "Add Woodpecker CI pipeline"
git push origin stage  # или prod
```

### 3. Смотрите результат в UI

1. **Repositories** → выберите репозиторий
2. **Pipelines** (вкладка) - видите запущенную сборку
3. Кликните на пайплайн → видите логи в реальном времени

**Статусы:**

- 🟡 **Pending** - ждет свободного агента
- 🔵 **Running** - выполняется
- 🟢 **Success** - успешно завершен
- 🔴 **Failure** - ошибка

---

## Работа в UI: Просмотр логов

### Просмотр логов конкретной сборки

1. **Repositories** → репозиторий
2. **Pipelines** → выберите пайплайн (кликните на строку)
3. Видите все шаги (steps)
4. Кликните на шаг → видите логи этого шага

### Перезапустить пайплайн

1. Откройте пайплайн
2. Кнопка **Restart** (справа сверху)
3. Пайплайн запустится заново с теми же параметрами

---

## Работа в UI: Ручной запуск

Запустить пайплайн вручную без пуша в Git:

1. **Repositories** → репозиторий
2. Кнопка **Trigger pipeline** (справа сверху, иконка play)
3. Выберите:
   - **Branch:** `stage` или `prod`
   - **Variables:** (опционально) можно передать переменные
4. **Trigger**

Пайплайн запустится для выбранной ветки.

---

## Работа в UI: Мониторинг агентов

Проверить сколько агентов работает:

1. **Admin Settings** (иконка шестеренки, правый верх)
2. **Agents** (вкладка)
3. Видите список агентов:
   - **Status:** Online/Offline
   - **Platform:** linux
   - **Capacity:** сколько задач выполняет

---

## Масштабирование через Lens

Увеличить количество агентов (больше параллельных сборок):

1. **Lens** → **Helm** → **Releases**
2. Найдите `woodpecker` → правый клик → **Upgrade**
3. В values.yaml измените:

```yaml
agent:
  replicas: 5 # было 2
```

4. **Upgrade**

Проверка:

- **Workloads** → **Pods** → должно быть 5 агентов

---

## Удаление через Lens

1. **Lens** → **Helm** → **Releases**
2. Найдите `woodpecker`
3. Правый клик → **Delete**
4. Подтвердите удаление

**Удалить данные (если нужно):**

1. **Storage** → **Persistent Volume Claims**
2. Namespace: `woodpecker`
3. Выберите PVC → правый клик → **Delete**

---

## Troubleshooting через Lens

### Сервер не запускается

1. **Workloads** → **Pods** → `woodpecker-server-xxxxx`
2. Кликните на под → **Logs**
3. Смотрите ошибку

**Частые причины:**

- Неверный GitHub Client ID/Secret
- Неверный host URL (должен совпадать с доменом)

### Агенты не подключаются

1. **Workloads** → **Pods** → `woodpecker-agent-xxxxx`
2. Кликните на под → **Logs**
3. Должна быть строка: `successfully connected to server`

### Пайплайн висит в Pending

**Причина:** нет свободных агентов.

**Решение:**

- Увеличьте количество агентов (см. "Масштабирование")
- ИЛИ подождите пока текущие сборки завершатся

---

## Полезные функции UI

### Badge (бейдж статуса) для README

1. **Repositories** → репозиторий
2. **Settings** → **Badge** (вкладка)
3. Скопируйте Markdown код
4. Вставьте в `README.md` вашего репо

Будет отображаться статус сборки: ✅ или ❌

### Cron (периодические сборки)

1. **Repositories** → репозиторий
2. **Settings** → **Cron** (вкладка)
3. **New cron job**:
   - **Name:** `nightly-build`
   - **Branch:** `stage`
   - **Schedule:** `0 2 * * *` (каждую ночь в 2:00)
4. **Save**

### Webhooks (автоматически настроены)

GitHub webhook настроен автоматически при Enable репозитория.

**Проверить:**

1. GitHub → ваш репо → **Settings** → **Webhooks**
2. Должен быть webhook на `https://woodpecker.stroy-track.ru/api/hook`
